...
- **TRA exemption (SCA-RTS Art. 18 / Art. 19):** fraud rate must be at or below the SCA-RTS Appendix reference rate for the ETV used. ETV bands and maximum permitted fraud rates: up to £85 → 0.13%; up to £220 → 0.06%; up to £440 → 0.01%. **(SCA-RTS Art. 19)** A £250 transaction falls in the "up to £440" band — the PSP's reference fraud rate must be at or below **0.01%** for the TRA exemption to apply. A fraud rate of 0.04% exceeds this threshold — **the TRA exemption cannot be applied**. The PSP must apply SCA for this transaction unless a **different exemption applies** — for example, the low-value contactless exemption, recurring fixed-amount transaction exemption, or trusted beneficiary exemption. Where no alternative exemption applies, SCA is required. Recalculate at least every 90 days. Notify FCA and cease immediately if rate exceeded. **(SCA-RTS Art. 18; SCA-RTS Art. 19)**
