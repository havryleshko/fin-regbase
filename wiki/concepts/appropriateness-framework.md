...
- **Suitability is a three-dimension test (knowledge/experience + financial situation + investment objectives); appropriateness is a one-dimension test (knowledge and experience only) (COBS 9A.2.1R; COBS 10A.2.1R).** Suitability applies to personal recommendations and portfolio management — the firm must assess all three dimensions before recommending. Appropriateness applies to non-advised (execution-only) services for complex products — the firm assesses only whether the client has the knowledge and experience to understand the product's risks; financial situation and investment objectives are not assessed. The two are mutually exclusive.
- **Suitability is a hard gate — no recommendation without sufficient information (COBS 9.2.6R; COBS 9A.2.13R).** If the client fails suitability, the firm must not make the recommendation. Appropriateness is a softer gate — if the client fails or information is unavailable, the firm must warn the client **(COBS 10A.2.1R; COBS 10.3.3G; COBS 10A.3.3G)** but the client can still ask the firm to proceed and the firm has a discretion to execute.
