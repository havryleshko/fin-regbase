# Wiki Index

Master index of all pages in the fin-regbase knowledge base.
Every page is listed with a one-line summary, jurisdiction, and source count.
Claude Code reads this file first on every query to identify relevant pages.

---

## UK — FCA Principles for Businesses (PRIN)

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/prin1-2-introduction-principles.md](summaries/prin1-2-introduction-principles.md) | PRIN 1–2: application/purpose; fit and proper link; group activities (P3/P4/P11); worldwide scope; breach consequences (fault standard per Principle); ECP classification (PRIN 1 Annex 1 — eligible types and requirements; not available for credit/PSR/EMD); all 12 Principles table | 4 |
| [summaries/prin2a-consumer-duty.md](summaries/prin2a-consumer-duty.md) | PRIN 2A: three cross-cutting rules (good faith, avoid foreseeable harm, enable financial objectives); PRIN 2A.2.26R (cross-cutting rules exhaust P12); four outcome rules; PRIN 2A.8.4R (annual governing body outcomes review); PRIN 2A.9.17R (FCA notification duty in distribution chains); PRIN 2A.9.4R (information sharing obligation); PRIN 2A.11.4R (due diligence on business acquisition); closed products; guarantors as retail customers | 12 |
| [summaries/prin3-rules-about-application.md](summaries/prin3-rules-about-application.md) | PRIN 3: P12 activity scope (PRIN 3.2.6R — 8 listed sourcebooks); P12 outer boundary (PRIN 3.2.8R); distribution chain scoping (PRIN 3.2.7R — determines/materially influences); P6/P7 displacement (PRIN 3.2.10R); territorial scope ("located" vs "habitually resident"); ECP unavailability for credit/PSR/EMD (PRIN 3.4.-1R); guarantors as retail customers (PRIN 3.4.3AR); no private right of action (PRIN 3.4.4R); EMIs/PIs/RAISPs within PRIN (PRIN 3.1.1AR) | 4 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/prin-principles-overview.md](concepts/prin-principles-overview.md) | All 12 Principles (hard Rules, PRIN 2.1.1R); territorial reach table (P4/P11 worldwide; P12 "located in UK"); P6/P7 displacement by P12 (PRIN 3.2.10R); group activities (P3/P4/P11); P12 activity scope; ECP business (P7 non-misleading only; ECP unavailable for credit/PSR/EMD); no private right of action (PRIN 3.4.4R); EMIs/PIs/RAISPs explicitly caught (PRIN 3.1.1AR) | 6 |

---

## UK — FCA Consumer Duty

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/ps22-9-policy-statement.md](summaries/ps22-9-policy-statement.md) | PS22/9 policy statement: introduces Principle 12, 3 cross-cutting rules, 4 outcomes; implementation timeline; scope; governance requirements | 1 |
| [summaries/fg22-5-guidance.md](summaries/fg22-5-guidance.md) | FG22/5 operational guidance companion to PS22/9: good/poor practice examples, sludge, supervisory questions, sector scope table, four-driver culture model | 1 |
| [summaries/fg21-1-guidance.md](summaries/fg21-1-guidance.md) | FG21/1 vulnerability guidance (Feb 2021): HLRC taxonomy, TEXAS model, policy/practice gap, vulnerability MI catalogue; pre-Duty but cross-referenced by FG22/5 and remains in force | 1 |
| [summaries/dear-ceo-letter-wealth-stockbroking-2023.md](summaries/dear-ceo-letter-wealth-stockbroking-2023.md) | FCA Dear CEO letter (Nov 2023): wealth management/stockbroking sector; dual priority of financial crime prevention and Consumer Duty; named sector-specific failures; supervisory posture shift | 1 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/consumer-duty-overview.md](concepts/consumer-duty-overview.md) | High-level overview of the Duty's three-layer structure, scope, sector-specific retail customer definitions, evidence requirement, and implementation dates | 2 |
| [concepts/consumer-principle.md](concepts/consumer-principle.md) | Principle 12 ("act to deliver good outcomes"): replaces Principles 6 & 7 (PRIN 3.2.10R); proportionality; SM&CR Individual Conduct Rule 6; outer scope boundary (PRIN 3.2.8R); cross-cutting rules exhaust P12 (PRIN 2A.2.26R); guarantors as retail customers (PRIN 3.4.3AR); no private right of action (PRIN 3.4.4R); FCA notification duty in chains (PRIN 2A.9.17R) | 8 |
| [concepts/consumer-duty-cross-cutting-rules.md](concepts/consumer-duty-cross-cutting-rules.md) | Three cross-cutting rules: good faith (incl. bad-faith pattern catalogue), avoid foreseeable harm (incl. examples list), enable financial objectives (incl. signposting for declined customers) | 3 |
| [concepts/consumer-duty-products-services.md](concepts/consumer-duty-products-services.md) | Products & Services outcome: target market, lifecycle obligations, manufacturer/distributor split, PROD equivalence; supervisory questions; distribution chain information-flow failures | 4 |
| [concepts/consumer-duty-price-value.md](concepts/consumer-duty-price-value.md) | Price & Value outcome: fair value assessment, distribution chain responsibilities, closed book application, COLL/PROD equivalence; named poor practices incl. charging for undelivered services | 3 |
| [concepts/consumer-duty-consumer-understanding.md](concepts/consumer-duty-consumer-understanding.md) | Consumer Understanding outcome: communications must demonstrably support effective decisions; testing obligation; supervisory questions; financial promotions approvers; execution-only carve-out | 4 |
| [concepts/consumer-duty-consumer-support.md](concepts/consumer-duty-consumer-support.md) | Consumer Support outcome: no unreasonable barriers; sludge/positive friction; at-least-as-easy-to-exit rule; channel obligations; digital-only rules; outsourcing | 3 |
| [concepts/vulnerable-customers.md](concepts/vulnerable-customers.md) | Vulnerability as a cross-cutting lens: HLRC taxonomy; TEXAS model; policy/practice gap; vulnerability MI catalogue; four-driver culture model; GDPR special category basis; supervisory red lines | 4 |
| [concepts/vulnerability-drivers-and-taxonomy.md](concepts/vulnerability-drivers-and-taxonomy.md) | HLRC four-driver taxonomy (FG21/1 Table 1); vulnerability spectrum; scale data (46%/53%); "do not use the label"; TEXAS model; policy/practice gap; inclusive design; MI catalogue | 1 |
| [concepts/sludge-and-friction.md](concepts/sludge-and-friction.md) | Sludge (harmful friction) vs positive friction (beneficial friction): definitions, named examples, the at-least-as-easy-to-exit rule, what does not constitute an unreasonable barrier | 1 |

### Derived

| Page | Summary | Sources |
|------|---------|---------|
| [derived/fca-supervisory-questions-consumer-duty.md](derived/fca-supervisory-questions-consumer-duty.md) | All FCA supervisory questions for Consumer Duty, per outcome (Products & Services, Price & Value, Consumer Understanding, Consumer Support, Culture & Governance) | 1 |
| [derived/fca-consumer-duty-implementation-review-2024.md](derived/fca-consumer-duty-implementation-review-2024.md) | FCA's February 2024 post-go-live review: good and poor practice observed across all five areas; includes gamification, fraud victim support, vulnerability red lines, and "wait and see" failure mode | 1 |
| [derived/consumer-understanding-digital-notifications.md](derived/consumer-understanding-digital-notifications.md) | Compliance assessment: digital investment platform in-app notifications against the Consumer Understanding outcome; covers gamification, behavioural bias, timing, testing, execution-only carve-out | 6 |
| [derived/consumer-duty-product-lifecycle-checklist.md](derived/consumer-duty-product-lifecycle-checklist.md) | Manufacturer obligations across the full product lifecycle (design → testing → distribution → ongoing review → material change → withdrawal); hard rules vs expected practices; SM&CR record-keeping accountability; closed product obligations; PROD 3 / Consumer Duty equivalence | 6 |

---

## UK — Data Protection / ICO

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/ico-guide-lawful-basis-2026.md](summaries/ico-guide-lawful-basis-2026.md) | ICO Guide to Lawful Basis (April 2026 update): seven Article 6 bases post-DUA Act; recognised legitimate interest (Art. 6(ea)) five pre-approved purposes; rights linkage table; consent trap; new purpose compatibility; dual condition for special category/criminal offence data; ICO guidance citation type noted | 1 |
| [summaries/ico-guide-data-protection-principles.md](summaries/ico-guide-data-protection-principles.md) | ICO Guide to Data Protection Principles (Art. 5): all seven principles; Art.83(5)(a) highest-tier fines; principle (b) updated March 2026 for DUA Act (Annex 2 nine compatible purposes, two-track reuse); invisible processing; accuracy nuances (historical records, mistakes, opinions, credit scores); storage limitation (no prescribed periods, pseudonymisation not exempt, archiving exception); DUA Act review status noted | 8 |
| [summaries/ico-guide-individual-rights.md](summaries/ico-guide-individual-rights.md) | ICO Guide to Individual Rights (v0.0.2, May 2023): all 8 data subject rights; Art. 12 cross-cutting framework (1-month calendar deadline, 28-day safe target, verbal requests, manifestly unfounded/excessive definitions); Art. 22 automated decision-making (3 permitted grounds, special category restriction, DPIA, human review); Art. 21 right to object (absolute direct marketing, suppression not erasure, compelling legitimate grounds); Art. 17 erasure (6 triggers, 5 exemptions, legal obligation and legal claims protect CDD records); Art. 13/14 privacy notice mandatory content; portability (observed data in, inferred out); SAR coverage — see ico-guide-subject-access | 1 |
| [summaries/ico-guide-subject-access.md](summaries/ico-guide-subject-access.md) | ICO Guide to Subject Access (top-level overview, clipped 2026-04-14): SAR recognition (verbal/written/social media; no specific form); third-party SAR authority mechanics (burden on third party); child SAR competence framework; clarification pause (distinct from extension — clock stops, supplementary info still due within 1 month); SAR response = copy + supplementary information (≈ Art. 13/14 content); DPA 2018 Schedules 2/3 SAR-specific exemptions (17 named categories; most relevant: crime/taxation, legal professional privilege, regulatory functions, management information, negotiations, confidential references); enforced SAR = criminal offence; special cases (credit, health, educational, social work) — sub-page detail not ingested | 1 |
| [summaries/ico-guide-data-security.md](summaries/ico-guide-data-security.md) | ICO Guide to Data Security (Arts. 5(1)(f) + 32): risk-based "appropriate" standard; Art. 32 four sub-obligations (encryption/pseudonymisation as examples; CIA triad + resilience; restore availability in timely manner; mandatory testing obligation for all organisations); PCI-DSS sector interaction (informs but ≠ UK GDPR compliance); processor security — "sufficient guarantees," contractual Art. 32 requirement, audit rights; Art. 32(4) staff training incl. DPA 2018 criminal liability; DUA Act under review | 1 |
| [summaries/ico-guide-personal-data-breaches.md](summaries/ico-guide-personal-data-breaches.md) | ICO Personal Data Breaches guide (Arts. 33–34): two-tier risk assessment (likely risk → Art. 33 ICO notification; likely HIGH risk → Art. 34 individual notification; no threshold → Art. 33(5) document only); 72-hour clock from "becoming aware"; Art. 33(3) four mandatory content elements; Art. 33(4) phased notification; Art. 33(2) processor notification obligation; Art. 34 "without undue delay," protective advice element, ICO power to compel; Art. 33(5) mandatory breach register for ALL breaches; Art. 83(4) fine tier £8.7m/2% (lower tier); PECR 72h updated by DUA Act Aug 2025 (first PECR content in wiki) | 1 |
| [summaries/ico-guide-accountability-governance.md](summaries/ico-guide-accountability-governance.md) | ICO Guide to Accountability and Governance (Arts. 5(2), 24–25, 28, 30, 33–36, 37–39, 40–43): eight accountability measures; DPbD as legal requirement (updated Feb 2026 for DUAA); mandatory processor contracts (Art. 28 minimum terms); RoPA obligation (Art. 30) and additional documentation (consent, breach register); DPIAs legally required for high-risk processing (Arts. 35–36); DPO mandatory/voluntary conditions and governance requirements; codes of conduct and certification (voluntary); privacy management framework; under ICO review for DUAA | 1 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/uk-gdpr-lawful-basis.md](concepts/uk-gdpr-lawful-basis.md) | UK GDPR Article 6 lawful bases: all seven bases including recognised legitimate interest (DUA Act 2025); necessity test; rights matrix (erasure/portability/object per basis); consent trap prohibition; new purpose compatibility assessment; Article 9 special category dual condition; Article 10 criminal offence data dual condition | 1 |
| [concepts/uk-gdpr-data-protection-principles.md](concepts/uk-gdpr-data-protection-principles.md) | UK GDPR Article 5 — all seven principles: lawfulness/fairness/transparency (three sub-obligations, invisible processing); purpose limitation (Annex 2 nine compatible purposes, two-track reuse, five-factor assessment); data minimisation (ceiling and floor); accuracy (historical records, mistakes, opinions, credit scores); storage limitation (no prescribed periods, pseudonymisation not exempt, archiving exception); integrity and confidentiality (Art. 32 four sub-obligations; see uk-gdpr-security for detail); accountability | 9 |
| [concepts/uk-gdpr-individual-rights-overview.md](concepts/uk-gdpr-individual-rights-overview.md) | UK GDPR individual rights: table of all 8 rights with basis dependency and absolute/qualified status; Art. 12 cross-cutting request framework in full (calendar month deadline, 28-day target, verbal requests, manifestly unfounded two-test definition, excessive definition, fee rules, ID proportionality); rectification (Art. 16); restriction (Art. 18 — 4 triggers, storage-only default, technical methods); portability (Art. 20 — consent/contract + automated; observed in, inferred out; CSV/XML/JSON) | 1 |
| [concepts/uk-gdpr-automated-decision-making.md](concepts/uk-gdpr-automated-decision-making.md) | UK GDPR Art. 22: two-track framework (general profiling vs. solely automated with legal/similarly significant effects); Art. 4(4) profiling definition; "similarly significant" = auto credit refusal (ICO named example); 3 permitted grounds (contract, law, explicit consent); special category data extra restriction (explicit consent or substantial public interest only); DPIA mandatory; meaningful logic transparency; human review mechanism; general profiling obligations where Art. 22 doesn't apply; fintech applicability table | 1 |
| [concepts/uk-gdpr-right-to-object.md](concepts/uk-gdpr-right-to-object.md) | UK GDPR Art. 21: right to object applies to direct marketing, legitimate interests, public task, research/statistics; direct marketing absolute (no grounds to refuse; suppression not erasure); LI/public task qualified (compelling legitimate grounds test; burden on controller; substantial damage/distress strengthens individual's grounds); proactive notification at first communication; Art. 21(6) research restriction depends on public task variant | 1 |
| [concepts/uk-gdpr-right-to-erasure.md](concepts/uk-gdpr-right-to-erasure.md) | UK GDPR Art. 17: 6 triggers (no longer necessary; consent withdrawn; successful objection; unlawful processing; legal obligation; children's Art. 8 consent); 5 exemptions — legal obligation (b) and legal claims (e) directly protect CDD/AML records; partial erasure; third-party notification (publicly available data); backup/processor systems must also erase; contrast with restriction as interim response | 1 |
| [concepts/uk-gdpr-right-to-be-informed.md](concepts/uk-gdpr-right-to-be-informed.md) | UK GDPR Art. 13/14: 13 mandatory items (Art. 13 direct collection — at point of collection); Art. 14 adds categories and source of data (within 1 month / first communication / first disclosure — earliest); automated decision-making item mandatory for all profiling (not just Art. 22); meaningful logic/significance/consequences required; Art. 14 exceptions; delivery methods (layered, just-in-time, dashboards, icons) | 1 |
| [concepts/uk-gdpr-subject-access.md](concepts/uk-gdpr-subject-access.md) | UK GDPR Art. 15 / SAR: SAR recognition (no specific form/words/channel); third-party SAR mechanics (authority burden on third party; proportionate evidence); child SAR (competence-based — respond to child if understands rights; parent needs authorisation or best-interests basis); clarification pause (large data volumes; clock stops not extends; supplementary info still due within 1 month); SAR response = copy + supplementary information (≈ Art. 13/14); DPA 2018 Schedules 2/3 SAR exemptions (crime/taxation; legal professional privilege; management information; negotiations; confidential references); enforced SAR = criminal offence; third-party data in responses; special cases stub | 1 |
| [concepts/uk-gdpr-security.md](concepts/uk-gdpr-security.md) | UK GDPR Art. 32 security of processing: risk-based "appropriate" standard; four sub-obligations — (a) encryption/pseudonymisation as examples not requirements; (b) CIA triad (confidentiality/integrity/availability) + resilience; (c) restore availability in timely manner; (d) mandatory regular testing obligation for all organisations; organisational measures (risk assessment, policy, accountability structure); technical measures (physical + cybersecurity, Cyber Essentials as baseline); PCI-DSS interaction for card processing fintechs; processor security obligations (sufficient guarantees, Art. 32 contract requirement, audit rights — first Art. 28 content in wiki); Art. 32(4) staff training incl. DPA 2018 criminal liability for deliberate unauthorised access | 1 |
| [concepts/uk-gdpr-personal-data-breaches.md](concepts/uk-gdpr-personal-data-breaches.md) | UK GDPR Arts. 33–34 breach notification: definition (confidentiality/integrity/availability breaches); two-tier risk assessment decision tree (likely risk → Art. 33; likely HIGH risk → also Art. 34; no threshold → Art. 33(5) document only); Art. 33 mechanics (72-hour clock from "becoming aware," 4 mandatory content elements, phased notification, processor Art. 33(2) obligation); Art. 34 individual notification (higher "high risk" bar, no fixed clock, protective advice element, ICO power to compel); Art. 33(5) mandatory breach register for ALL breaches (no threshold, primary ICO audit target); Art. 83(4) fine tier £8.7m/2% (lower tier, distinct from Art. 83(5)(a)); parallel obligations table (PECR 72h post-DUA Act, eIDAS 24h, NIS); post-breach SAR surge contingency | 1 |
| [concepts/uk-gdpr-data-protection-by-design.md](concepts/uk-gdpr-data-protection-by-design.md) | UK GDPR Art. 25 DPbD: legal requirement (not best practice); "by design" = embed throughout lifecycle; "by default" = most privacy-protective option is the default (no indefinite retention, no broad access as default); named measures (minimisation, pseudonymisation, security features); risk-based and proportionate; updated 5 Feb 2026 for DUAA; DPIAs as primary DPbD evidence mechanism; fines at Art. 83(4) tier | 1 |
| [concepts/uk-gdpr-dpia.md](concepts/uk-gdpr-dpia.md) | UK GDPR Arts. 35–36 DPIAs: legally required before high-risk processing (three triggers: automated decisions with legal/significant effects; large-scale special category data; large-scale systematic monitoring); Art. 35(7) minimum content (description, necessity/proportionality, risk assessment, safeguards); DPO must be consulted (Art. 35(2)); prior ICO consultation mandatory where residual high risk cannot be mitigated (Art. 36); dual function = risk management + accountability evidence; ongoing obligation | 1 |
| [concepts/uk-gdpr-dpo.md](concepts/uk-gdpr-dpo.md) | UK GDPR Arts. 37–39 DPO: mandatory for public bodies, large-scale systematic monitoring, large-scale special category data processing; five tasks (inform/advise, monitor compliance, advise on DPIAs, cooperate with ICO, contact point for data subjects); three governance requirements (report to highest management, operate independently, adequate resources); conflict of interest prohibition; even if not mandatory — sufficient staff/skills/structures still required | 1 |
| [concepts/uk-gdpr-records-of-processing.md](concepts/uk-gdpr-records-of-processing.md) | UK GDPR Art. 30 RoPA: most organisations required (small-org exemption narrow; financial services almost always in scope); controller RoPA content (purposes, data subject/data categories, recipients, retention, transfers, security); processor RoPA; additional documentation obligations (consent records Art. 7(1); breach register Art. 33(5); DPIAs; compatibility assessments); RoPA as foundation for accuracy, storage limitation, security, and rights responses; ICO's primary audit document | 1 |
| [concepts/uk-gdpr-processor-contracts.md](concepts/uk-gdpr-processor-contracts.md) | UK GDPR Art. 28 controller–processor contracts: mandatory written contract before processing begins; eight mandatory minimum terms (instructions-only, confidentiality, Art. 32 security, sub-processor controls, rights assistance, compliance assistance, deletion/return, audit rights); "sufficient guarantees" due diligence pre-engagement; sub-processor rules (prior written authorisation; same obligations flow down; main processor remains liable); Art. 83(4) fine tier | 1 |

---

## UK — FCA Financial Crime

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/fcg3-money-laundering-terrorist-financing.md](summaries/fcg3-money-laundering-terrorist-financing.md) | FCG 3: full AML/CTF regulatory architecture; governance, MLRO, risk assessment, CDD, EDD triggers, SARs, record keeping, wire transfers, Travel Rule, enforcement case studies | 1 |
| [summaries/jmlsg-part-i-guidance.md](summaries/jmlsg-part-i-guidance.md) | JMLSG Part I (June 2023, Aug 2025 update): Treasury-approved AML/CTF guidance; SDD, DAML consent regime, tipping off, third-party reliance, risk factor weighting, PEP continuum, Annual Financial Crime Report | 1 |
| [summaries/fcg7-sanctions-asset-freezes-proliferation-financing.md](summaries/fcg7-sanctions-asset-freezes-proliferation-financing.md) | FCG 7 (April 2026): UK financial sanctions; OFSI; Consolidated List screening; asset freeze obligation; dual OFSI+FCA breach reporting; evasion detection; proliferation financing (MLR Reg 18A); RBS £5.6m case study | 1 |
| [summaries/fctr8-sanctions-systems-controls-russia.md](summaries/fctr8-sanctions-systems-controls-russia.md) | FCTR 8 (June 2023): thematic review of sanctions systems and controls post-Russia invasion; FCA Sanctions Screening Tool (SST); 5 themes: governance/MI, skills/resources, screening calibration, CDD/KYC, breach reporting timeliness; UK vs. OFAC divergence; horizon scanning obligation | 6 |
| [summaries/fca-cdd-findings-multi-firm-2026.md](summaries/fca-cdd-findings-multi-firm-2026.md) | FCA 2025 multi-firm CDD review (2026): alternative ID policy gap; EDD documentation differentiation requirement; periodic review timing in policy; January 2024 domestic PEP change; three-lines independence failure; sectors: asset management, crowdfunding, wholesale banking, CFDs, non-bank lenders | 1 |

### Derived

| Page | Summary | Sources |
|------|---------|---------|
| [derived/pi-emi-consumer-duty-and-sanctions-obligations.md](derived/pi-emi-consumer-duty-and-sanctions-obligations.md) | Derived comparison: Consumer Duty (retail scope only, per PSRs/EMRs) vs UK sanctions (all customers/transactions) for PIs and EMIs; three genuine tensions (freeze vs. support, tipping-off vs. transparency, screening friction vs. no unreasonable barriers); scope asymmetry table; PI AML gap (HMRC-supervised, FCG 3 doesn't apply, but FCG 7 does) | 6 |
| [derived/aml-kyc-onboarding-decision-tree.md](derived/aml-kyc-onboarding-decision-tree.md) | Sequential onboarding decision tree: sanctions screening (Stage 0) → SDD/Standard/EDD classification → four-step CDD → EDD (PEP/high-risk country/correspondent banking) → vulnerability and mental capacity check → SAR/DAML decision gate → onboarding approval → post-onboarding monitoring calibration; tipping-off analysis; DAML moratorium periods; record retention table | 7 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/aml-ctf-framework.md](concepts/aml-ctf-framework.md) | AML/CTF framework: regulatory architecture (MLR, SYSC, POCA, Terrorism Act), senior management governance, MLRO role and failure modes, risk assessment, SARs and personal liability, record keeping, CTF | 2 |
| [concepts/customer-due-diligence.md](concepts/customer-due-diligence.md) | CDD, EDD, SDD, third-party reliance, and monitoring: beneficial ownership (25% threshold), mandatory EDD triggers, source of wealth/funds, reliance conditions and pro-formas, transaction monitoring, Travel Rule; alternative ID policy requirement; EDD documentation differentiation; three-lines independence for CDD monitoring; January 2024 domestic PEP change | 3 |
| [concepts/simplified-due-diligence.md](concepts/simplified-due-diligence.md) | SDD: third CDD tier for low-risk situations; qualifying categories; permitted adjustments to timing/quantity/quality/monitoring; when SDD must stop; reliance limit | 1 |
| [concepts/sar-consent-tipping-off.md](concepts/sar-consent-tipping-off.md) | SARs operational framework: legal definitions of suspicion and reasonable grounds; DAML consent procedure (7-day/31-day/186-day); tipping-off criminal offences; permitted disclosure carve-outs; deposit-taker £3k threshold | 1 |
| [concepts/uk-financial-sanctions-framework.md](concepts/uk-financial-sanctions-framework.md) | UK financial sanctions regime: SAMLA, OFSI, Consolidated List, scope (all FCA-supervised firms incl. EMIs, PIs, cryptoassets), Principle 11 notification, contrast with AML scope | 1 |
| [concepts/sanctions-screening.md](concepts/sanctions-screening.md) | Sanctions screening: Consolidated List screening obligations, timing (onboarding + list updates), screening of directors/BOs, automated tool calibration and fuzzy matching, evasion detection, NECC alerts, RBS enforcement case | 1 |
| [concepts/asset-freezes-and-ofsi-licences.md](concepts/asset-freezes-and-ofsi-licences.md) | Asset freezes: immediate freeze obligation on designation; OFSI licence regime; dual breach reporting (OFSI mandatory + FCA under SUP 15.3); false positive escalation and audit trail; distinction from DAML consent | 1 |
| [concepts/proliferation-financing.md](concepts/proliferation-financing.md) | Proliferation financing: standalone MLR Reg 18A PF risk assessment; dual-use goods; trade finance EDD; Counter-Terrorism Act 2008 direction powers; JMLSG Part III as primary operational source | 1 |

---

## UK — FCA Payment Services

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/dear-ceo-letter-payments-portfolio-2023.md](summaries/dear-ceo-letter-payments-portfolio-2023.md) | FCA portfolio letter (March 2023): three outcomes (customers' money safe — safeguarding/prudential/wind-down; financial system integrity — AML/sanctions/fraud; customers' needs — Consumer Duty); three cross-cutting priorities (governance/agent oversight, SYSC 15A operational resilience, regulatory reporting); supervisory escalation warning | 1 |
| [summaries/fca-approach-payment-services-electronic-money.md](summaries/fca-approach-payment-services-electronic-money.md) | FCA Approach Document v7 (March 2026): primary operational guidance for PSRs 2017 and EMRs 2011; covers Ch 8 conduct, Ch 9 capital, Ch 10 safeguarding, Ch 13 reporting, Ch 17 open banking (PISP/AISP), Ch 18 operational risk, Ch 20 SCA; five firm types (Authorised PI, Small PI, Authorised EMI, Small EMI, RAISP) | 1 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/safeguarding-pis-emis.md](concepts/safeguarding-pis-emis.md) | Safeguarding under PSRs/EMRs: obligation on receipt; two methods (segregation vs insurance/comparable guarantee); named segregation account requirements (no shared accounts, acknowledgement letter terms); daily reconciliation; annual audit; PESAR and asset pool insolvency priority; NOT002 FCA notification | 2 |
| [concepts/psr-capital-requirements.md](concepts/psr-capital-requirements.md) | Capital requirements for Authorised PIs and EMIs: initial capital thresholds (PI €20k/€50k/€125k; EMI €350k); ongoing own funds Methods A/B/C (PIs) and Method D (EMIs — 2% avg e-money); FCA ±20% adjustment power; own funds quality (CET1/AT1/T2) | 1 |
| [concepts/psr-sca-authentication.md](concepts/psr-sca-authentication.md) | Strong customer authentication (SCA): three-factor requirement (knowledge/possession/inherence — two from different categories); dynamic linking (amount + payee specific); 9 SCA-RTS exemptions including TRA with fraud rate thresholds and ETVs (£85/£220/£440); 5-minute inactivity; 5-failure block; quarterly monitoring | 1 |
| [concepts/psr-regulatory-reporting.md](concepts/psr-regulatory-reporting.md) | PSR/EMR regulatory reporting: 10+ mandatory forms (FSA056 PI capital annual; REP017 fraud twice-yearly; REP018 op/security risk quarterly; REP020 dedicated interface quarterly; DISP 1AD complaints; REP-CRIM financial crime); event notifications NOT002–005; major operational/security incident 4-hour initial notification | 1 |
| [concepts/psr-open-banking-pisp-aisp.md](concepts/psr-open-banking-pisp-aisp.md) | Open banking framework: ASPSP non-discrimination obligations; dedicated interface requirements; contingency mechanism and exemption criteria (SCA-RTS Art. 33(6), 3-month wide usage test); AISP 90-day consent reconfirmation; PISP hard restrictions (no funds, no order modification); sensitive payment data prohibitions; NOT003 access denial; NOT005 interface problems | 1 |
| [concepts/psr-unauthorised-transaction-liability.md](concepts/psr-unauthorised-transaction-liability.md) | Unauthorised and incorrectly executed transactions: immediate refund obligation (next business day); £35 customer excess cap; fraud delay mechanism (4 business days, reg 86(2A)-(2D)); burden of proof on PSP; 13-month notification limit; PISP liability chain; SCA liability shift; incorrect unique identifier rules | 1 |
| [concepts/wind-down-planning.md](concepts/wind-down-planning.md) | Wind-down planning obligation (Approach Document §3.73–3.76): triggers, step-by-step procedure, realistic timelines, cost/cash analysis; TR22/1 thematic review reference; four named common plan failures | 2 |
| [concepts/psr-emi-prudential.md](concepts/psr-emi-prudential.md) | Prudential risk management for PIs/EMIs: holding capital above minimum, liquidity risk identification, scenario planning, stress testing, risk appetite, FG20/1 reference; see psr-capital-requirements for calculation methods | 3 |
| [concepts/agent-distributor-oversight.md](concepts/agent-distributor-oversight.md) | Agent and distributor oversight: agents (FCA registration mandatory, PI/EMI responsibility), distributors (EMI notification only, no payment services), due diligence and ongoing monitoring obligations, communications obligations through agents/distributors | 2 |
| [concepts/operational-resilience.md](concepts/operational-resilience.md) | SYSC 15A operational resilience (in force 31 March 2022): important business services, impact tolerances, 31 March 2025 mapping and testing deadline, third-party dependency management, FCA Register accuracy | 2 |

---

## UK — FCA Dispute Resolution (DISP)

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/disp1-treating-complainants-fairly.md](summaries/disp1-treating-complainants-fairly.md) | DISP 1: scope (FSMA firms + PSPs/EMIs not FSMA-authorised); MiFID complaints extend to ECPs (DISP 1.1A.3R); root cause analysis hard rule (DISP 1.3.3R — extends to non-complained products); named SMF holder for DISP compliance (DISP 1.3.7R); 3-day track + summary resolution communication (DISP 1.5); dual timelines (8 weeks standard / 15 BD EMD-PSD / 35 BD exceptional); forwarding; time barring; records (3yr/5yr); standard bi-annual reporting DISP 1.10 (Annex 1R); PSP/EMI annual reporting DISP 1.10B (Annex 1AD — all users not just eligible complainants) | 13 |
| [summaries/disp-app5-motor-finance-dca-complaints.md](summaries/disp-app5-motor-finance-dca-complaints.md) | DISP App 5: modified complaint handling for motor finance DCA complaints (pre-28 Jan 2021) and non-DCA complaints; 8-week response deadline paused (DCA: 11 Jan 2024 – 31 May 2026; non-DCA credit: 20 Dec 2024 – 31 May 2026); FOS referral window extended to 15 months (or fixed 30 Jul 2026) for final responses sent during extended period, reverts to 6 months from 30 Jan 2026; final response Annex 3R wording substituted ("fifteen months"); record retention obligation to 11 April 2031 regardless of complaint receipt | 14 |
| [summaries/disp2-fos-jurisdiction.md](summaries/disp2-fos-jurisdiction.md) | DISP 2: four-gate FOS jurisdiction test (activity type, territorial scope, eligible complainant, timeliness); CJ scope (FCA firms, PSPs, EMIs, CBTL firms, credit reference agencies, finance platforms); VJ; territorial scope (UK establishment — complainant need not be UK-based); eligible complainant summary (7 categories, doubt = eligible, PEP identification extension, APP fraud extension, regulated-firm exclusion); DISP 2.8 dual time limits (6 months from final response; 6 years/3 years absolute); 6-month clock contingent on FOS notice in final response | 8 |
| [summaries/disp3-fos-complaint-procedures.md](summaries/disp3-fos-complaint-procedures.md) | DISP 3: FOS determination standard ("fair and reasonable" — law + FCA rules + guidance + codes + good industry practice at the time, DISP 3.6.1R); provisional assessment → determination procedure; dismissal grounds (post-9 July 2015: 5 grounds only — no "fair offer" dismissal); test case referral; evidence powers (exclude admissible, include inadmissible, adverse inferences); award types: money (financial loss, distress, pain, reputational damage — whether or not court would award), interest, costs, direction (broader than court order); award cap from 1 April 2026: £455,000 post-April 2019 acts / £205,000 pre-April 2019 acts (CPI-adjusted); acceptance = final and binding; silence = deemed rejection; respondent delay adds to award; compliance mandatory, court-enforceable | 8 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/complaints-handling-framework.md](concepts/complaints-handling-framework.md) | DISP 1 operational rules: dual timeline (8 weeks / 15-35 BD EMD-PSD); 3-day track + summary resolution communication; root cause analysis hard rule (DISP 1.3.3R — extends to non-complained products); named SMF-level DISP 1 compliance owner; DISP 1.10B PSP/EMI annual reporting (all users). Also covers FOS-side: "fair and reasonable" determination standard (DISP 3.6.1R — includes good industry practice); provisional assessment; award cap from 1 April 2026 (£455,000 / £205,000); distress/pain/suffering awards where court wouldn't; direction broader than court order; acceptance = binding; respondent delay adds to award | 11 |
| [concepts/fos-eligible-complainants.md](concepts/fos-eligible-complainants.md) | DISP 2.7 eligible complainant framework: seven inclusion categories (consumer, micro-enterprise, charity < £6.5m income, trustee < £5m net assets, CBTL consumer, small business < £6.5m turnover at referral, guarantor); doubt = eligible rule (DISP 2.7.5G — FOS decides, not respondent); regulated-firm exclusion (DISP 2.7.9R — PSPs/EMIs not eligible for complaints about own regulated activity; professional clients/ECPs excluded in CJ); PEP identification eligible complainant (DISP 2.7.7AR); APP fraud eligible complainant extension (DISP 2.7.7G(2B)); dual FOS time limits (DISP 2.8.2R — 6 months from final response; 6yr/3yr absolute); 6-month clock contingent on FOS notice in response (DISP 2.8.3G) | 7 |

---

## UK — SM&CR (Senior Managers and Certification Regime)

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/fca-guide-solo-regulated-firms.md](summaries/fca-guide-solo-regulated-firms.md) | FCA guide for solo-regulated firms (July 2019, Part One): three tiers (Limited Scope/Core/Enhanced); SMFs by tier; Core and Enhanced PRs (tables); SoRs; Duty of Responsibility; 12-week rule; outsourcing accountability; Certification Functions (8); FIT test; criminal records check rules; regulatory references (6-year lookback, NDA prohibition, no time limit for serious misconduct); Conduct Rules (Individual 1–6; SC1–SC4); breach reporting (7-day SM; annual REP008); PI/EMI exclusion | 1 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/smcr-senior-managers-regime.md](concepts/smcr-senior-managers-regime.md) | SM&CR Senior Managers Regime: PI/EMI exclusion (FSMA-only scope); three tiers; SMFs by tier (6 Core, 3 Limited Scope, 17 Enhanced); SoRs; Duty of Responsibility (FSMA s.66A statutory defence); Core PRs (a/b/b-1/d/z); Enhanced PRs (12); Enhanced-only obligations (Overall Responsibility, Responsibilities Map SYSC 25, Handover Procedures); 12-week rule; outsourcing accountability; Head of Legal exclusion | 1 |
| [concepts/smcr-certification-regime.md](concepts/smcr-certification-regime.md) | SM&CR Certification Regime: firm-issued annual certification (not FCA approval); 8 Certification Functions (CF29, proprietary traders, CASS Operational Oversight, qualification functions, client dealing, supervisors, Material Risk Takers, algorithmic trading); FIT test (honesty/integrity, competence, financial soundness); criminal records checks (mandatory SMs/NEDs, discretionary for Certification staff); regulatory references (6-year lookback; NDA prohibition; no time limit for serious misconduct) | 1 |
| [concepts/smcr-conduct-rules.md](concepts/smcr-conduct-rules.md) | SM&CR Conduct Rules: Individual Rules 1–5 (integrity; due care; openness with regulators; customer interests; market conduct) + Rule 6 (Consumer Duty addition); Senior Manager Rules SC1–SC4 (effective control, regulatory compliance, appropriate delegation, FCA/PRA disclosure); ancillary staff exclusion; breach reporting (7 business days for SM; annual REP008 for all others) | 1 |

---

## UK — MiFID II / PROD

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/mifid-ii-product-governance-review-2021.md](summaries/mifid-ii-product-governance-review-2021.md) | FCA multi-firm review (Feb 2021): product governance at 8 asset managers; negative target market near-universal failure; distributor information flow breakdown; product-level scenario testing gaps; AFM Board NED challenge failures; record keeping SM&CR risk | 1 |
| [summaries/fca-costs-charges-disclosure-review-2019.md](summaries/fca-costs-charges-disclosure-review-2019.md) | FCA multi-firm review (Feb 2019): transaction cost calculation failures (arrival price misuse, anti-dilution levy misuse, outsourcing oversight, underlying fund classification); PRIIPs KID vs. marketing material non-contradiction rule; UCITS KIID material transaction cost obligation; disclosure accessibility; AMC prohibition (TR 14-07) | 1 |
| [summaries/fca-pfof-supervisory-report-2019.md](summaries/fca-pfof-supervisory-report-2019.md) | FCA PFOF supervisory report (Apr 2019): 15 firms reviewed; exclusive vs. non-exclusive liquidity framework; four conditions for dual-sided charging; five named pretextual PFOF structures; overseas affiliate routing as circumvention; systems and controls deficiencies including sub-1% sampling | 1 |
| [summaries/fca-assessing-suitability-review-2017.md](summaries/fca-assessing-suitability-review-2017.md) | **[HISTORICAL — pre-MiFID II]** FCA suitability review (May 2017): 1,142 files, 656 firms; 93.1% suitable sector-wide; two root causes (risk profiling, replacement business); 41.7% unacceptable disclosure concentrated in initial charges disclosure; three-part framework (COBS 6.1A, COBS 14, COBS 9.4); firm-type differentials | 1 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/prod-product-governance.md](concepts/prod-product-governance.md) | PROD 3 product governance framework for investment manufacturers: positive and negative target market (PROD 3.2.10R), conflicts of interest, product scenario testing (PROD 3.2.13R), costs disclosure, distributor due diligence and information flow, AFM board/NED obligations, record keeping and SM&CR personal liability | 1 |
| [concepts/costs-charges-disclosure.md](concepts/costs-charges-disclosure.md) | Costs and charges disclosure framework: PRIIPs KID non-contradiction rule, UCITS KIID material transaction cost obligation, four transaction cost calculation failure modes (arrival price, anti-dilution levy, outsourcing, underlying fund classification), disclosure accessibility as standalone breach, AMC vs OCF terminology | 1 |
| [concepts/payment-for-order-flow.md](concepts/payment-for-order-flow.md) | PFOF framework: definition and harm mechanism; exclusive vs. non-exclusive liquidity legal distinction; ROFR as the key marker; four cumulative conditions for dual-sided charging; five named pretextual structures; overseas affiliate routing circumvention; SYSC 10 systems and controls requirements | 1 |
| [concepts/advice-suitability-cobs9.md](concepts/advice-suitability-cobs9.md) | **[pre-MiFID II baseline]** COBS 9 suitability for personal recommendations: three-dimension test; two root causes of unsuitable advice (risk profiling, replacement business); three-part advisory disclosure framework (COBS 6.1A initial charges, COBS 14 product, COBS 9.4 suitability report); specific initial disclosure failure modes; sector risk segmentation | 1 |
| [concepts/appropriateness-framework.md](concepts/appropriateness-framework.md) | Appropriateness vs suitability framework: suitability (COBS 9/9A) = advised services, three-dimension test, hard gate (cannot transact without sufficient info); appropriateness (COBS 10/10A) = non-advised services, knowledge and experience only, soft gate (warning + firm discretion); execution-only non-complex exemption — 4 criteria (non-MiFID) vs 6 criteria (MiFID; adds no profile-altering clause, no implicit exit charges); suitability displaces appropriateness; targeted support (COBS 9B) displaces both; product-regime mapping table; interaction rules | 4 |

---

## UK — COBS (Conduct of Business Sourcebook)

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/cobs9-suitability.md](summaries/cobs9-suitability.md) | COBS 9 (non-MiFID / non-insurance-based): three-dimension suitability test (knowledge/experience, financial situation, investment objectives — COBS 9.2.1R); hard prohibition on recommending without sufficient information (COBS 9.2.6R); prohibition on discouraging disclosure (COBS 9.2.4R); churning — individually suitable trades can be collectively unsuitable (COBS 9.3.2G); suitability report mandatory content, timing rules and triggers (COBS 9.4.1R–9.4.8R); pension transfer one-page summary (COBS 9.4.11R); record retention tiers: indefinitely (pension transfer/conversion/opt-out/FSAVC), 5 years (life policy/personal pension/DC occupational), 3 years (other) (COBS 9.5.2R); insistent client regime: communications required, own-words acknowledgement, record-keeping (COBS 9.5A); basic advice on stakeholder products (COBS 9.6) | 6 |
| [summaries/cobs9a-suitability-mifid.md](summaries/cobs9a-suitability-mifid.md) | COBS 9A (MiFID / IBIP): professional client assumptions (per se = knowledge/experience + financial situation; elective = knowledge/experience only — COBS 9A.2.5R); risk profiling tool reliability as hard Rule (fit-for-purpose, limitations identified and mitigated — COBS 9A.2.9R); switching cost-benefit analysis — must demonstrate benefits > costs (COBS 9A.2.18R/9A.2.18AR); automated/semi-automated systems — suitability responsibility not reduced (COBS 9A.2.23R); periodic suitability at least annually + mandatory pre-service disclosure of frequency, triggers, reassessment scope (COBS 9A.3.6R; 9A.3.9R–10R); legal person / representative written policy required — split dimensions by client type (COBS 9A.2.15R); IBIP records retained for duration of relationship (COBS 9A.4.3R); suitability report before conclusion for retail MiFID advice; distance communication exception | 3 |
| [summaries/cobs10-appropriateness-non-mifid.md](summaries/cobs10-appropriateness-non-mifid.md) | COBS 10 (non-MiFID appropriateness for non-advised services): knowledge and experience only test; scope covers non-readily realisable securities, speculative illiquid securities, derivatives, warrants, LTAF units, UK RIE crypto ETNs, P2P, qualifying cryptoassets (direct offer financial promotion trigger — includes registered persons); does not apply where personal recommendation / targeted support given; two mandatory warnings (product inappropriate; insufficient info) — both standardised format; client may proceed after warning at firm's discretion; execution-only non-complex exemption (4-criterion test — at client initiative, non-complex instrument, informed disclosure, conflicts compliance); one assessment per service not per transaction; 5-year record retention; 5 product-specific question matrices (non-readily realisable securities 13 matters; P2P 14 matters; LTAF 9 matters; qualifying cryptoassets 12 matters including volatility/cyber risks/FOS-FSCS limits; UK RIE crypto ETNs 12 matters) | 5 |
| [summaries/cobs10a-appropriateness-mifid.md](summaries/cobs10a-appropriateness-mifid.md) | COBS 10A (MiFID / IBIP appropriateness for non-advised services): professional client assumption — may assume K&E for services/products for which classified; 6-criterion non-complex test (2 additional vs COBS 10 — no profile-altering clause/trigger; no implicit exit charges making instrument illiquid); non-complex IBIP exemption (5-criterion test — guaranteed minimum maturity value ≥ premiums paid; no profile-altering trigger; surrender options at available value; no disproportionate exit charges; no opacity structure); four-item mandatory MiFID record including firm's own decision whether to accept client request to proceed after warning (COBS 10A.7.2R); IBIP records: duration of relationship minimum; reliance on another firm's appropriateness or suitability assessment; bundled package assessment required; reliance standard "aware or ought to be aware" | 3 |

---

## UK — MAR (Market Abuse Regulation)

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/mar1-market-abuse.md](summaries/mar1-market-abuse.md) | MAR 1 FCA guidance on UK MAR behaviours: intent not required (MAR 1.2.3G); "made public" four-factor test incl. above-average-expertise extension (MAR 1.2.12G–1.2.15G); insider dealing — front-running/pre-positioning named (MAR 1.3.2G(2)); unlawful disclosure — selective analyst briefing named; confidentiality+necessity safe harbour (MAR 1.4.5G); manipulating transactions — legitimate reasons + AMP double test; abusive squeeze — willingness to lend as key indicator + multilateral default risk + spot-forward divergence (MAR 1.6.11G); dissemination — Chinese wall as defence (MAR 1.8.5G); three statutory safe harbours (buy-back, price stabilisation, AMPs) | 9 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/market-abuse-framework.md](concepts/market-abuse-framework.md) | UK market abuse framework overview: two-layer structure (UK MAR + FCA MAR Sourcebook); five prohibited behaviours table (insider dealing, unlawful disclosure, manipulating transactions, manipulating devices, dissemination); intent not required as core principle; inside information four-element definition; "made public" test incl. expert-derivable extension; three statutory safe harbours; legitimate reasons + AMP double test for manipulation; firm obligations under SYSC (conflicts, STORs, SM&CR) | 2 |

---

## UK — FCA Consumer Credit (CONC)

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/conc1-application-purpose.md](summaries/conc1-application-purpose.md) | CONC 1: scope (10 credit-related activity types), jurisdictional reach (habitual residence test, extraterritorial), MCOB boundary (land-secured = MCOB), AR liability (s.39(4) FSMA), Consumer Duty confirmation (CONC 1.1.4G), 7 financial difficulty indicators | 3 |
| [summaries/conc2-conduct-of-business-general.md](summaries/conc2-conduct-of-business-general.md) | CONC 2: active election for optional products (2.2.6R — pre-ticked boxes prohibited); credit broker 22-item unfair practices catalogue (2.5.8R); HCSTC PCW rank-by-total-amount-payable rule (2.5A); unsolicited credit token prohibition (2.9.2R); mental capacity presumption + 9 indicators (2.10); remuneration risk detection obligation (2.11.4R); distance marketing disclosure + durable medium (2.7); e-commerce firm identification (2.8) | 11 |
| [summaries/conc3-financial-promotions.md](summaries/conc3-financial-promotions.md) | CONC 3: CFNM five-element standard (3.3.1R); HCSTC verbatim risk warning; 8-element representative example + 3 APR triggers incl. speed/ease of application; 5 unsecured + 3 secured restricted expressions; verbatim land-secured warnings (standard/consolidation/equity-release); debt counsellor 16-item disclosure + look-alike website/SEO prohibition; CCA s.49 criminal canvassing offence; cannot approve interactive promotions (3.11.2R) | 12 |
| [summaries/conc4-pre-contractual-requirements.md](summaries/conc4-pre-contractual-requirements.md) | CONC 4: adequate explanation obligation (4.2.5R — 5 elements, cannot be waived, no adequacy acknowledgement); product-specific explanations (HCSTC unsuitability, BNPL promotional rate conditions); broker payment-detail gate (4.4.3R — info notice + customer confirmation); DCA prohibition (4.5.6R); pre-contract CPA 11-item disclosure (4.6.2R); unfair pressure/loan-inflation ban (4.8) | 8 |
| [summaries/conc5-responsible-lending.md](summaries/conc5-responsible-lending.md) | CONC 5: creditworthiness assessment (5.2A lenders, 5.5A P2P); three hard triggers + cumulative increase rule; two-track credit risk/affordability risk; five-condition affordability test; income/expenditure assessment; self-declaration insufficient; security exclusion; running-account worst-case drawdown; guarantor obligations; written policies (governing body approval); P2P dual obligation and lender disclosure rule; credit broker suitability obligation (5.4) | 3 |
| [summaries/conc6-post-contractual-requirements.md](summaries/conc6-post-contractual-requirements.md) | CONC 6: pre-arrears monitoring duty + written policy (6.7.2R/6.7.3CR); credit card minimum repayment floor (interest+fees+1% outstanding, 6.7.5R); credit limit and interest rate freeze when at risk (6.7.7R/6.7.10R); "at risk" definition (6.7.11G — 2+ arrears/plan/DMP discussion); BNPL promotional rate reminder obligation (6.7.16AR); HCSTC two-refinancing cap (6.7.23R) | 7 |
| [summaries/conc7-arrears-default-recovery.md](summaries/conc7-arrears-default-recovery.md) | CONC 7: arrears/default policies (written, governing-body reviewed); forbearance hard rule + individual circumstances; interest freeze on active repayment arrangements (7.3.5G R); debt counsellor 30-day suspension; CPA two-strike rule (HCSTC); mental capacity mandatory suspension; disputed debt mandatory suspension; statute barred rules (6yr E&W, 5yr Scotland — Scotland: debt ceases to exist); post-assignment lender responsibility; P2P arrears notices (7.17–7.19) | 17 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/conc-scope-and-application.md](concepts/conc-scope-and-application.md) | CONC scope: 10 regulated activity types (lending, broking, debt collecting, P2P, consumer hiring, etc.); habitual residence extraterritorial rule; MCOB boundary and broking carve-out; CONC 1.2.2R employee/agent compliance obligation; AR = principal firm's acts by statute; Consumer Duty explicitly applies via CONC 1.1.4G | 2 |
| [concepts/conc-financial-difficulty-indicators.md](concepts/conc-financial-difficulty-indicators.md) | CONC 1.3.1G: 7 financial difficulty indicators (consecutive missed minimums; adverse credit file; CCJs; inability to repay from disposable income; consecutive missed repayments; DMP agreement; DMP discussions); constructive knowledge standard; trigger for CONC 6 and 7 obligations; CONC 6.7.11G "at risk" definition (2+ consecutive arrears/plan/DMP discussion); overlap with FG21/1 vulnerability framework | 2 |
| [concepts/conc-creditworthiness-assessment.md](concepts/conc-creditworthiness-assessment.md) | CONC 5.2A/5.5A/5.4: three hard assessment triggers + cumulative increase trap; two-track (credit risk + affordability risk); five-condition affordability test; self-declaration insufficient; security exclusion from affordability; running-account worst-case drawdown assumption; guarantor = additional not substitute; written policies governing-body approved; P2P dual obligation + lender disclosure; credit broker suitability obligation | 3 |
| [concepts/conc-discretionary-commission-arrangements.md](concepts/conc-discretionary-commission-arrangements.md) | CONC 4.5.6R DCA prohibition: three named DCA types (IUA/IDA/scaled); commission disclosure (4.5.3R prominent; 4.5.3AR cost impact; 4.5.4R on-request amount); accrued commissions rule; motor finance context | 3 |
| [concepts/conc-cpa-rules.md](concepts/conc-cpa-rules.md) | CONC 7.6 CPA rules: pre-conditions for exercise (explained + in-agreement); proportionality + forbearance obligation; financial difficulty mandatory suspension (CONC 7.6.7R); two-strike rule for HCSTC (CONC 7.6.12R); instalment two-strike rule (7.6.13R); no partial amounts for HCSTC (7.6.14R); customer cancellation rights (7.6.16R–7.6.17R); applies to BNPL via CONC 7.1.3A R | 2 |
| [concepts/conc-debt-recovery-treatment.md](concepts/conc-debt-recovery-treatment.md) | CONC 7 treatment standards: written arrears/vulnerability policy obligation; forbearance hard rule (individual circumstances); interest freeze on active arrangements (7.3.5G R mandatory); charges limits (7.7); mental capacity mandatory suspension (7.10.1R); post-assignment lender responsibility (7.12.2R–7.12.3G); disputed debt suspension + burden on firm (7.14.1R, 7.14.4R); statute barred rules E&W/Scotland (7.15); data accuracy before pursuit (7.13.4R) | 8 |
| [concepts/conc-mental-capacity-guidance.md](concepts/conc-mental-capacity-guidance.md) | CONC 2.10: presumption of capacity (2.10.4G); 9 indicators of limitation (2.10.8G); duty to assist before excluding (2.10.5G); high scrutiny on creditworthiness assessment; unsuitable credit prohibition even if affordable (2.10.20G); written procedures required; links to CONC 5.2A, 7.2.1R, 7.10.1R | 4 |
| [concepts/conc-broker-unfair-practices.md](concepts/conc-broker-unfair-practices.md) | CONC 2.5.8R 22-item catalogue: TPS/automated call prohibition; premium rate complaint lines (15-min cap); securing worse rate/more credit for broker's gain; add-on insurance pressure; consolidation against interest; payment account access without authorisation; data passing to unauthorised firms; HCSTC PCW rank-by-total-amount-payable; results/promotions separation | 4 |
| [concepts/conc-financial-promotions-cfnm.md](concepts/conc-financial-promotions-cfnm.md) | CONC 3.3.1R CFNM five-element standard: identifiable; accurate; balanced/risks; sufficient for target audience; not concealing. Comparisons (1B) must be fair/balanced/meaningful. No-credit-regardless prohibition (3.3.3R). BNPL promotional rate omission rule (3.3.11AR). Overdraft = "borrowing" (3.3.12R). Nine named unfair practice examples (3.3.10G). Prominent = draws attention of average customer in target audience | 3 |
| [concepts/conc-representative-example-apr.md](concepts/conc-representative-example-apr.md) | CONC 3.5: representative example triggered by any cost/rate indication; 8 mandatory elements at equal prominence; representative = 51% threshold; 3 APR-only triggers (restricted audience / favourable comparison / incentive to apply incl. speed/ease); APR formatting rules; compulsory ancillary services statement; security statement; 5 restricted expressions unsecured + 3 secured (3.6.8R) | 2 |

### Derived

| Page | Summary | Sources |
|------|---------|---------|
| [derived/conc-financial-difficulty-trigger-map.md](derived/conc-financial-difficulty-trigger-map.md) | Signal-to-obligation map across CONC: 7 CONC 1.3.1G indicators; CONC 6.7.11G at-risk threshold (2 consecutive misses / arrangement / DMP discussions — any one sufficient); 8 named signals (arrears, two consecutive misses, CPA failure by next working day, customer provides evidence, DMP/formal arrangement, CCJ/insolvency, mental capacity, disputed debt) each mapped to exact CONC rules triggered; interest freeze automatic conditions; HCSTC two-strike cap; cumulative signal escalation table; Scottish statute-barred distinction | 12 |

---

## UK — FCA Systems and Controls (SYSC)

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/sysc9-record-keeping.md](concepts/sysc9-record-keeping.md) | SYSC 9: two-track architecture (non-MiFID general duty — retain "as long as relevant"; MiFID/common platform — mandatory record type categories, 5-year minimum); tamper-proof storage (SYSC 9.1.2-AR — prevents firm-level alteration, FCA-accessible, reconstitutable); AISP/PISP record-keeping for credit institutions (SYSC 9.2 — 4 data categories, 5-year from 13 Jan 2018 baseline); SM&CR personal liability from record-keeping gaps | 6 |
| [concepts/sysc10-conflicts-of-interest.md](concepts/sysc10-conflicts-of-interest.md) | SYSC 10: strict identify → manage → disclose sequence (disclosure = last resort, SYSC 10.1.9AR); hard identification duty (SYSC 10.1.3R — prospective, firm vs client and client vs client); 6 conflict types (SYSC 10.1.4R — incl. inducements as Type 5, PFOF basis); conflicts register (SYSC 10.1.6R) + annual senior management report (SYSC 10.1.6AAR — separate obligations); written conflicts policy (SYSC 10.1.10R) with 6 named measures (SYSC 10.1.11R) and annual review (SYSC 10.1.11AAR); Chinese walls (SYSC 10.2.2R) as statutory defence under FSA 2012 ss.89–91 and s.138D | 12 |

---

## UK — FCA Cryptoassets

### Summaries

| Page | Summary | Sources |
|------|---------|---------|
| [summaries/ps23-6-cryptoasset-financial-promotions.md](summaries/ps23-6-cryptoasset-financial-promotions.md) | PS23/6 (eff. 8 Oct 2023): qualifying cryptoassets classified as RMMIs; four lawful communication routes (authorised person, s21 approver, MLR Art. 73ZA, FPO exemption); PIs/EMIs authorised under PSRs/EMRs only are not authorised persons; FPO exemptions disapplied (Art. 48 HNW, Art. 50A self-certified sophisticated, Art. 51, Art. 61); Consumer Duty applies to authorised approvers/communicators only (not yet MLR businesses); s21 gateway requires FCA permission, 7-day notification, quarterly attestations; MLR registration ~£10,000; Article 73ZA MLR businesses subject to all consumer journey rules and extended FSMA enforcement powers | 6 |

### Concepts

| Page | Summary | Sources |
|------|---------|---------|
| [concepts/cryptoasset-financial-promotions-framework.md](concepts/cryptoasset-financial-promotions-framework.md) | PS23/6 consumer journey operative rules: mandatory DOFP sequence (8 steps in order); 24-hour cooling-off (first-time investors only); personalised risk warning format (digital/non-digital); absolute incentives ban (no shareholder benefit exemption); three client categories (Restricted Investor, HNW, Certified Sophisticated — self-certified sophisticated disapplied); 12-month declaration expiry; appropriateness anti-gaming rules (different questions each attempt; 24-hour lock-out from second fail; no specific answer disclosure; no encouragement to retake; time counts toward cooling-off); existing vs new customer decision tree (Figure 3) | 5 |
